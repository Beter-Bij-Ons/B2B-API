...
function createInvoiceChristmass(invoice) {
  ...
  generateFooter(doc);
  const fileName = 'B2C-Invoice/'+newID+'.pdf';
...
function sendOrderEmail(filename,invoice) {
  ...
  admin.database().ref('/Prod/Invoice-kerstmarkt-B2C/'+invoice.dbid).update({
  ...
  mailOptions.attachments =  [{
    filename: invoice.invoice_nr+"_bon.pdf",
  ...
  return mailTransport.sendMail(mailOptions).then(() => {
    ...
    const mailOptionsIternal ={
      ...
      subject: 'Bon verstuurd',
      html:'Er is een bon aangemaakt'
  };
...
function generateHeader(doc) {
  doc
  .image("logo-streek.png",50, 45, { width: 120 })
  .fillColor("#444444")
  .fontSize(20)
    .text("", 110, 57)
    .fontSize(10)
