...
function createInvoiceGeitengeluk(invoice) {
  ...
  generateFooter(doc);
  const fileName = 'bf4f01bd-3dac-4cb4-a68b-462d96f0e508/'+newID+'.pdf';
...
function sendOrderEmail(filename,invoice) {
  ...
  const mailOptions = {
    ...
    to: email,
    bcc:'lisanne@geitengeluk.nl,joppe@beterbijons.nl',
    subject: 'Bestelling bij Geitengeluk',
...
function generateHeader(doc) {
  ...
  .fontSize(20)
    .text("", 110, 57)
    .fontSize(10)
    .text("Geitengeluk", 200, 50, { align: "right" })
    .text("Vrij 5", 200, 65, { align: "right" })
    .text("5853 EJ, Siebengewald", 200, 80, { align: "right" })
    .text("+31(0)645919766", 200, 95, { align: "right" })
    .text("lisanne@geitengeluk.nl", 200, 110, { align: "right" })
    .moveDown();
}
...
console.log('greater:' + position);
  ...
  generateTableRow(
    ...
    "Verzendkosten",
    "",
    invoice.shippingType == 'Afhalen' ?'Afhalen':'€ 7,00'
  );
